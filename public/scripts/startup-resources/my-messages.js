...
(function(code){
  ...
  let replyBtn = $('.reply-btn');
  let cancelBtn = $('.cancel-btn');
  let deleteBtn = $('.delete-btn');
  let cancelDeleteBtn = $('.cancel-delete-btn');
  let msgActions = $('.message-actions');
  let firstMsg = $('#my-messages-ul').children('.message-tab').first();
  let onLoadMsgId = firstMsg.next().val();
  ...
  $.ajax({
    'url': `/startup-resources/my-messages/query/${onLoadMsgId}`,
    'type': 'GET',
    'dataType' : 'json',
  ...
  messageTabs.each(function(){
    $(this).on('click', function(){
      ...
      $.ajax({
        'url': `/startup-resources/my-messages/query/${msgId}`,
        'type': 'GET',
        'dataType' : 'json',
        'success': function(data){
          msgWindow.append(data.data)
